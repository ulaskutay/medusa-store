  ...
  return sdk.client
    .fetch<{
      ...
      query: {
        ...
        offset,
        region_id: region?.id,
        fields: process.env.NEXT_PUBLIC_MARKETPLACE_MODE
          ...
          : '*variants.calculated_price,+variants.inventory_quantity,*variants,+options,+images,+collection,+categories',
        ...queryParams
      },
      headers,
    ...
    .then(({ products: productsRaw, count }) => {
      const products = productsRaw.filter(
        (product) =>
          !product.seller || product.seller?.store_status !== 'SUSPENDED'
      );
